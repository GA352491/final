...
def notify(request):
    ...
    noti_count = Stock.objects.filter(stock_available__lte=F('quantity') * 0.37).count()
    print(notification)
    context = {'notification': notification, 'noti_count': noti_count}

    return render(request, 'notify.html', context)


def general_bill(request):
    a = Invoice.objects.all()
    form = Form5()
    if request.method == 'POST':
    ...
    context = {'form': form, 'a': a}
    return render(request, 'generalbill.html', context)
...
def test_stock(request):
    ...
    return render(request, 'test_stock.html', context)
...
def test_customer_list(request):
    ...
    return render(request, 'test_customer.html', context)

def test_customer(request, pk):
    customer = Customer.objects.get(id=pk)
    ord = Invoice.objects.filter(customer_name_id=pk).count()
    # orders = Customer.objects.filter(id=pk)
    orders1 = Invoice.objects.filter(customer_name_id=pk)
    ...
    context = {'customer': customer, 'ord': ord, 'orders1': orders1, 'myFilter': myFilter, 'e': e}
    return render(request, 'test_cus_view.html', context)